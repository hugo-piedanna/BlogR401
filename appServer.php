        ...
        if(isset($payload)){
            if($payload->type === "publisher"){
                if(isset($postedData["content"])){
                    ...
                    if($addArticle){
                        deliver_response(200, "Article ajouté", listArticle($payload->type));
                    }else{
                        deliver_response(502, "Erreur avec la requête envoyée à la base de données", NULL);
                    }
                    ...
                    if(isset($postedData["id_article"])){
                        ...
                        if($articleVote == 1){
                            ...
                            if($listeArticles != null){
                                deliver_response(200, "Le vote a été appliqué", listArticle($payload->type));
                            }else{
                                deliver_response(502, "Erreur avec la requête envoyée à la base de données", NULL);
                            }
        ...
        if(isset($payload)){
            if($payload->type === "publisher"){
                ...
                if(isset($postedData["id_article"])) {
                    $deleteArticle = deleteArticle($postedData["id_article"]);
                    if ($deleteArticle == 1) {
                        deliver_response(200, "L'article a été supprimé", listArticle($payload->type));
                    } else if($deleteArticle == 0){
                        deliver_response(400, "L'article n'a pas pu être supprimé", $postedData["id_article"]);
                    }else{
                        deliver_response(502, "Erreur avec la requête envoyée à la base de données", NULL);
                    }
                }else{
                    deliver_response(400, "Vous devez renseigner l'id d'un article", NULL);
                }
            }else{
                deliver_response(403, "Vous n'avez pas la permission de supprimer cette article", NULL);
            }
        }else{
            deliver_response(403, "Vous devez être connecté", NULL);
        }
